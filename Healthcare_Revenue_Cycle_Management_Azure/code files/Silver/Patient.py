...
# Reading Hospital H1 patient data 
df_hos_h1 = spark.read.csv("/mnt/bronze/aduet-hospital-h1/patients.csv", header=True)
df_hos_h1 = (
    df_hos_h1.withColumn("datasource", f.lit("hos-h1")).withColumn("DOB",f.to_date(f.col("DOB"), "M/d/yyyy"))
                    .withColumn("ModifiedDate",f.to_date(f.col("ModifiedDate"), "M/d/yyyy"))
)

df_hos_h1.createOrReplaceTempView("patients_hosh1")

# Reading Hospital H2 patient data 
df_hos_h2 = spark.read.csv("/mnt/bronze/aduet-hospital-h2/patients.csv", header=True)
df_hos_h2 = (
    df_hos_h2.withColumn("datasource", f.lit("hos-h2")).withColumn("DOB",f.to_date(f.col("DOB"), "M/d/yyyy"))
                    .withColumn("Updated_Date",f.to_date(f.col("Updated_Date"), "M/d/yyyy"))
)
                    
df_hos_h2.createOrReplaceTempView("patients_hosh2")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from patients_hosh1

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from patients_hosh2

# COMMAND ----------

# MAGIC %sql
# MAGIC /* Common Data Model for both hospitals */
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS silver_patients (
...
# MAGIC %sql
# MAGIC select count(*),Patient_Key from silver_patients
